...
const weightChartData = computed(() => ({
  ...
  datasets: [{
    label: 'Вес (кг)',
    data: filteredMeasurements.value.map(m => m.weight),
    borderColor: '#4CAF50',
    tension: 0.1
  }]
}));

const bodyCompositionChartData = computed(() => ({
  labels: filteredMeasurements.value.map(m => formatDate(m.date)),
  datasets: [
    {
      label: 'Жировая масса (%)',
      data: filteredMeasurements.value.map(m => m.bodyFatPercentage),
      borderColor: '#FF5252',
      tension: 0.1,
      yAxisID: 'percentage'
    },
    {
      ...
      data: filteredMeasurements.value.map(m => m.musclePercentage),
      borderColor: '#2196F3',
      tension: 0.1,
      yAxisID: 'percentage'
    },
    {
      label: 'Костная масса (%)',
      data: filteredMeasurements.value.map(m => m.bonePercentage),
      borderColor: '#4CAF50',
      tension: 0.1,
      yAxisID: 'percentage'
    },
    {
      label: 'Вода (%)',
      data: filteredMeasurements.value.map(m => m.waterPercentage),
      borderColor: '#00BCD4',
      tension: 0.1,
      yAxisID: 'percentage'
    },
    {
      ...
      data: filteredMeasurements.value.map(m => m.bodyFatMass),
      borderColor: '#FF8A80',
      tension: 0.1,
      yAxisID: 'mass',
      hidden: true
    },
    {
      ...
      data: filteredMeasurements.value.map(m => m.muscleMass),
      borderColor: '#82B1FF',
      tension: 0.1,
      yAxisID: 'mass',
      hidden: true
    },
    {
      ...
      data: filteredMeasurements.value.map(m => m.boneMass),
      borderColor: '#69F0AE',
      tension: 0.1,
      yAxisID: 'mass',
      hidden: true
    ...
    {
      label: 'Вода (кг)',
      data: filteredMeasurements.value.map(m => m.waterMass),
      borderColor: '#84FFFF',
      tension: 0.1,
      yAxisID: 'mass',
      hidden: true
    }
  ].filter(dataset => dataset.data.some(value => value !== undefined)) // Only show datasets with values
}));
...
const measurementsChartData = computed(() => ({
  ...
  datasets: [
    {
      label: 'Грудь (см)',
      data: filteredMeasurements.value.map(m => m.measurements?.chest),
      borderColor: '#9C27B0',
      tension: 0.1
    },
    {
      label: 'Талия (см)',
      data: filteredMeasurements.value.map(m => m.measurements?.waist),
      borderColor: '#FF9800',
      tension: 0.1
    },
    {
      label: 'Бедра (см)',
      data: filteredMeasurements.value.map(m => m.measurements?.hips),
      borderColor: '#795548',
      tension: 0.1
    }
  ]
